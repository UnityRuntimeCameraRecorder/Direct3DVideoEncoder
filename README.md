...
Used by [UnityRuntimeCameraRecorder](https://github.com/UnityRuntimeCameraRecorder/UnityRuntimeCameraRecorder) for GPU video encoding. The DLL itself does not depend on Unity or FFmpeg.
...
For Unity integration and camera/audio/MP4 recording, use [UnityRuntimeCameraRecorder](https://github.com/UnityRuntimeCameraRecorder/UnityRuntimeCameraRecorder).
...
`Direct3DVideoEncoderStartWithConstantQP(texture, width, height, fps, codec, qp, callback)` applies the following settings. The options variant also accepts an explicit preset and concurrent-encoding mode. UnityRuntimeCameraRecorder selects P5 normally and P4 for concurrent independent outputs.
